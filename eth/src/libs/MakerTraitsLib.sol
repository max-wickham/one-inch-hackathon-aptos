...
library MakerTraitsLib {
    ...
    function newMakerTraits(address sender, uint expiration, bool allowPartialFills_, bool hasExtension, uint nonce) public pure returns (MakerTraits) {
        ...
        if (sender != address(0)) {
            makerTraits |= uint160(sender) & _ALLOWED_SENDER_MASK;
        }
        if (expiration != 0) {
            makerTraits |= (expiration & _EXPIRATION_MASK) << _EXPIRATION_OFFSET;
        }
        if (allowPartialFills_) {
            makerTraits &= ~_NO_PARTIAL_FILLS_FLAG; // Clear the flag
        } else {
            makerTraits |= _NO_PARTIAL_FILLS_FLAG; // Set the flag
        }
        if (hasExtension) {
